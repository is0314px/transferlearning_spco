    ...
    for iter in xrange(iteration): #iteration == 200
        ...
        if ((iter + 1) % iteration) == 0:
            ...
            for ei, env_name in enumerate(env_names):
                ...
                plt.savefig("gibbs_result/sigverse_result/spcotransfer19_"+repr(data_set_num-1)+"/Visualize_learning_result/spatial_concept_"+env_name+"/per_"+repr(x)+".png",dpi=300) #spatial_concept
                plt.close()
            ...
            for c in range(Stick_large_L):
                ...
                plt.tight_layout()
                plt.ylim(0,1)
                plt.savefig("gibbs_result/sigverse_result/spcotransfer19_"+repr(data_set_num-1)+"/Visualize_learning_result/spatial_concept_global/per_"+repr(per)+"/concept_"+repr(c)+".png") #spatial_concept
                plt.close()
            #"""
