...
while per <= MAX_PER:
    ...
    for i, nd in enumerate(name_dic):
        if i > 6: #場所を表す単語が名前辞書で6番目以降であるため
            Names.append(nd)

    label_idx = [0,3,[1,2,9],8,6,5,4,7] #index of name label
    #entrance, toilet, bedroom, bath, living-room, dining, kitchen, washroom
    ...
    for i, name in enumerate(Names):
        ...
        for pos in sample_pos:
            ...
            if name == "entrance":
                ...
                if (-50 <= x and x <= 150) and (-480 <= y and y <= -360):
                ...
                if (-200 <= x and x <= -50) and (-160 <= y and y <= 0):
                    cnt_vector[name_dic.tolist().index(name)] += 1
            
            elif name == "bedroom":
                Costs = []
                for j in range(len(idx)):
                    cost = (((label_pos[idx[j]][0] - x)**2) + ((label_pos[idx[j]][1] - y)**2))**0.5
                    Costs.append(cost)
                cost = np.min(Costs) #３つのbedroomのgroun_truthとのcostの最小値
                cost_vector[name_dic.tolist().index(name)] += cost

                if (-590 <= x and x <= -50) and (-470 <= y and y <= -160) or (-590 <= x and x <= -200) and (-160 <= y and y <= 330) or ((100 <= x and x <= 150) and (-350 <= y and y <= -160)) or ((150 <= x and x <= 590) and (-480 <= y and y <= -160)):
                    cnt_vector[name_dic.tolist().index(name)] += 1
                ...
                if ((350 <= x and x <= 590) and (-160 <= y and y <= 0)):
                    cnt_vector[name_dic.tolist().index(name)] += 1
                ...
                if ((350 <= x and x <= 590) and (0 <= y and y <= 330)):
                    cnt_vector[name_dic.tolist().index(name)] += 1
                ...
                if ((60 <= x and x <= 300) and (140 <= y and y <= 330)):
                    cnt_vector[name_dic.tolist().index(name)] += 1
                ...
                if ((-200 <= x and x <= 60) and (50 <= y and y <= 330)):
                    cnt_vector[name_dic.tolist().index(name)] += 1
                ...
                if ((100 <= x and x <= 150) and (-160 <= y and y <= -70)) or ((150 <= x and x <= 350) and (-160 <= y and y <= 0)):
                    cnt_vector[name_dic.tolist().index(name)] += 1
