...
hyper_para = np.loadtxt("hyper_parameter/hyper_parameter_spcoa.txt",delimiter = " #") #read hyper parameters
...
alpha_n = hyper_para[0] #0.1
kappa_0 = hyper_para[1] #0.05
nu_0 = hyper_para[2] #10
gamma = hyper_para[3] #10
beta = hyper_para[4] #0.4
name_increment = hyper_para[5] #1
psi_0_x = hyper_para[6] #10.0
psi_0_y = hyper_para[7] #10.0
psi_0_sin = hyper_para[8] #0.5
psi_0_cos = hyper_para[9] #0.5
psi_0 = np.matrix([[psi_0_x,0.0,0.0,0.0],
                    [0.0,psi_0_y,0.0,0.0],
                    [0.0,0.0,psi_0_sin,0.0],
                    [0.0,0.0,0.0,psi_0_cos]])
epsilon = hyper_para[10] #0.1
...
def gibbs_sampling(per):
    ...
    for e, dir in enumerate(Training_list):
        ...
        G = nonpara_tool.stick_breaking(gamma,concept_num) #np.random.dirichlet(G_0 + gamma) #initialize G
        G_set.append(G)
    ...
    theta_n = np.array([[[float(1.0)/NAME_DIM for i in range(NAME_DIM)] for j in range(concept_num)] for k in range(data_set_num)]) #initialize theta^n
    ...
    for iter in xrange(iteration): #iteration == 200
        ...
        for e in xrange(data_set_num):
            ...
            #theta_v_log = np.log(theta_v[e])
            theta_n_log = np.log(theta_n[e])
            pi_data = np.array([pi_e.T[R_t[d]] for d in range(data_num[e])])
            pi_data = np.log(pi_data)
            G_log = np.log(G_set[e])
            
            for i in range(concept_num):
                name_prob = name_set[e].dot(theta_n_log[i])
            ...
            #=============== Sample theta^n and pi and G =============== 
            total_name_e_set = [] #store the number of name
            ...
            for c in xrange(concept_num): 
                ...
                theta_n[e][c] = np.random.dirichlet(total_name_e) + 1e-100 #sample theta_n
                
                pi_set[e][c] = np.random.dirichlet(concept_posdist_count[c] + beta) #sample pi
            Total_name_e_set.append(total_name_e_set)

            G_set[e] = np.random.dirichlet(concept_count + gamma) + 1e-100 #sample G
        ...
        for n in range(NAME_DIM):
            for c in range(concept_num):   
                prob_n[n] += theta_n[e][c][n] * G_set[e][c]

        for c in range(concept_num):
            for n in range(NAME_DIM):
                t_t_prob = (theta_n[e][c][n] * G_set[e][c]) / (prob_n[n] * G_set[e][c])
                t_t_prob = (theta_n[e][c][n] * G_set[e][c]) * math.log(t_t_prob) #P(n_t,C_t|Theta) * log()
                
                t_f_prob = ((1.0-theta_n[e][c][n]) * G_set[e][c]) / ((1.0 - prob_n[n]) * G_set[e][c])
                if t_f_prob == 0.0:
                    t_f_prob = 1e-100
                t_f_prob = ((1.0-theta_n[e][c][n]) * G_set[e][c]) * math.log(t_f_prob) #P(n_t_ber,C_t|Thera) * log()
                
                f_t_prob = ((1.0-G_set[e][c]) * theta_n[e][c][n]) / (prob_n[n] * (1.0-G_set[e][c]))
                if f_t_prob == 0.0:
                    f_t_prob = 1e-100
                f_t_prob = ((1.0-G_set[e][c]) * theta_n[e][c][n]) * math.log(f_t_prob) #P(n_t,C_t_ber|Theta) * log()
                
                f_f_prob = ((1.0-G_set[e][c]) * (1.0-theta_n[e][c][n])) / ((1.0-prob_n[n]) * (1.0-G_set[e][c]))
                if f_f_prob == 0.0:
                    f_f_prob = 1e-100
                f_f_prob = ((1.0-G_set[e][c]) * (1.0-theta_n[e][c][n])) * math.log(f_f_prob) #P(n_t_ber,C_t_ber|Theta) * log()
            ...
            if max_of_mutual_info[n] < epsilon:
                for c in range(concept_num): 
                    for e in range(data_set_num):
                        theta_n[e][c][n] = 1e-100

        for e in range(data_set_num): #normalize
            for c in range(concept_num):
                Sum = np.sum(theta_n[e][c])
                for n in range(NAME_DIM):
                    theta_n[e][c][n] = theta_n[e][c][n] / Sum
        ...
        if ((iter + 1) % iteration) == 0:
            ...
            #learning result. You can confilm result of clustering and word distribution (\theta_n)
            """
...
:
            ...
            plt.savefig("gibbs_result/sigverse_result/spcoa+MI/Visualize_learning_result/spatial_concept_new/per_"+repr(x)+".png",dpi=300) #spatial_concept
            plt.close()
            #"""
            ...
            for i in range(data_set_num):
                ...
                np.savetxt(Out_put_dir+"/dataset"+repr(i)+"/theta_n.csv",theta_n[i])              
