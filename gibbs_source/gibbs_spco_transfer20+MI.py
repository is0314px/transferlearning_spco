...
def gibbs_sampling(per):
    ...
    theta_v = np.array([[[float(1.0)/VISION_DIM for i in range(VISION_DIM)] for j in range(concept_num)] for k in range(data_set_num)]) #initialize theta^v
    ...
    for iter in xrange(iteration): #iteration == 200
        ...
        for e in xrange(data_set_num):
            ...
            for i in range(concept_num):
                vision_prob = vision_set[e].dot(theta_v_log[i])
            ...
            #=============== Sample theta^v and theta^n and pi and G =============== #additional part
            total_vision_e_set = [] #store the number of vision (visual feature)
            ...
            for c in xrange(concept_num): 
                ...
                total_vision_e = total_vision_e + phi_v[c] * delta_v # + alpha_v
                total_vision_e_set.append(total_vision_e)
                theta_v[e][c] = np.random.dirichlet(total_vision_e) + 1e-100 #sample theta_v

                total_name_e = BoF.bag_of_feature(name_e_c,NAME_DIM)
                total_name_e = total_name_e + phi_n[c] * delta_n #+ alpha_n
        ...
        if ((iter + 1) % iteration) == 0:
            ...
            #learning result. You can confilm result of clustering and word distribution (\theta_n, \phi_n)
            """
            ...
            for ei, env_name in enumerate(env_names):
                ...
                plt.savefig("gibbs_result/sigverse_result/spcotransfer20+MI_"+repr(data_set_num-1)+"/Visualize_learning_result/spatial_concept_"+env_name+"/per_"+repr(x)+".png",dpi=300) #spatial_concept
                plt.close()

            for c in range(Stick_large_L):
                for ei, env_name in enumerate(env_names):
                    e = env_index[ei]
                    plt.bar(name_dic,theta_n[e][c],tick_label=name_dic,align="center",color=color[c])
                    plt.xticks(rotation='vertical')
                    plt.tight_layout()
                    plt.ylim(0,1)
                    plt.savefig("gibbs_result/sigverse_result/spcotransfer20+MI_"+repr(data_set_num-1)+"/Visualize_learning_result/spatial_concept_"+env_name+"/per_"+repr(per)+"/concept_"+repr(c)+".png") #spatial_concept
                    plt.close()
                ...
                plt.tight_layout()
                plt.ylim(0,1)
                plt.savefig("gibbs_result/sigverse_result/spcotransfer20+MI_"+repr(data_set_num-1)+"/Visualize_learning_result/spatial_concept_global/per_"+repr(per)+"/concept_"+repr(c)+".png") #spatial_concept
                plt.close()
            #"""
            ...
            for i in range(data_set_num):
                ...
                np.savetxt(Out_put_dir+"/dataset"+repr(i)+"/theta_v.csv",theta_v[i])
